...
- **3つの人格の調整**: メルキオール（科学者）、バルタザール（母）、カスパー（女）それぞれのプロンプト、モデル、温度感を個別に設定可能。
- **管理画面**: 右上の「ADMIN」タブ内の「PERSONAS」から、ブラウザ上でリアルタイムに性格や思考ロジックをカスタマイズ・保存できます。
- **テンプレート**: `personas.json` を直接編集することで、システム全体のペルソナ定義を永続的に変更可能です。
